            ...
            */
            ...
            sourceCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat('/me Bot je naprvaljen od strane ' + botCreator + ', ali ga sada odrzava ' + botMaintainer + ".");
                    }
                }
            },
            ...
            fortunecookieCommand: {
                ...
                getFcookie: function (chat) {
                    var c = Math.floor(Math.random() * basicBot.chat.fcookies.length);
                    return basicBot.chat.fcookies[c];
                },
                functionality: function (chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(subChat(basicBot.chat.fortunecookie, {name: chat.un, fortune: this.getFcookie()}));
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(basicBot.chat.selffortuneccookie, {name: name}));
                            }
                            else if (user.username === chat.un) {
                                return API.sendChat(subChat(basicBot.chat.selffortuneccookie, {name: name}));
                            }
            ...
            discordCommand: {
                ...
                functionality: function (chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                    if (!bmbot1.commands.executable(this.rank, chat)) return void (0);
                    else {
                        if (typeof bmbot1.settings.discord === "string")
                            API.sendChat(subChat(bmbot1.chat.discord, {link: bmbot1.settings.discord}));
                    }
                }
            },
